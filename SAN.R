...
run_intermediate_annealing_process = function(env_full,already_sampled,n,theta,nspp,nswap = 1,
  ...
  for(i in 1:number_of_iterations) {
    ...
    still_prop = still
    swap1 = sample(n,nswap)
    swap2 = sample(length(still),nswap)
    ...
    if(prs==5000){
      svMisc::progress(100,progress.bar = T)
      break
